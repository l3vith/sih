...
import { Activity, AlertTriangle, ArrowDownRight, ArrowUpRight, Bell, BrainCircuit, ChevronDown, CircleHelp, Crosshair, Database, FileScan, FileText, Gauge, Layers3, MapPinned, Menu, Maximize2, Network, PanelLeftClose, Search, Send, Settings2, Sparkles, Target, Upload, X, Zap } from 'lucide-react'
...
const mapStyle: StyleSpecification = {
  version: 8, sources: {
    'indian-basemap': {
      ...
      tileSize: 256,
      maxzoom: 19,
      attribution: '© Esri — Source India data: Survey of India, NRSC',
    },
  },
  layers: [
...
function RealMap({ radius, onSelect, fullscreen }: { radius: number; onSelect: (id: string) => void; fullscreen: boolean }) {
  ...
  useEffect(() => {
    ...
    const map = new MapLibreMap({ container: containerRef.current, style: mapStyle, center, zoom: 5.5, minZoom: 4, maxZoom: 15, attributionControl: false, renderWorldCopies: false }); map.addControl(new NavigationControl({ showCompass: true }), 'top-right')
    map.on('load', () => {
...
export default function App() {
  ...
  useEffect(() => {
    function handleFullscreenChange() { setFullscreen(!!document.fullscreenElement) }
  ...
  function renderView() {
    ...
    return <><div className="hero-grid"><div className={`panel map-panel ${fullscreen ? 'map-panel-fullscreen' : ''}`}><PanelHeader icon={<MapPinned size={16} />} title="NEARBY WELLS MAP" meta={`${wells.filter((well) => well.state === 'active' || Number.parseFloat(well.distance) <= radius).length} WELLS / ${radius}.0 KM`} />      <div className="map-toolbar"><button className="icon-button" aria-label="Center active well"><Crosshair size={15} /></button><button className="icon-button" aria-label="Toggle layers"><Layers3 size={15} /></button><label className="radius-control">RADIUS <input type="range" min="1" max="8" value={radius} onChange={(event) => setRadius(Number(event.target.value))} /><strong>{radius}.0 km</strong></label><button className="icon-button" aria-label="Toggle fullscreen" onClick={toggleFullscreen}>{fullscreen ? <X size={15} /> : <Maximize2 size={15} />}</button></div><RealMap radius={radius} onSelect={setSelectedWell} fullscreen={fullscreen} /></div><div className="panel depth-panel"><DepthPanel telemetry={telemetry} /></div><div className="panel document-panel"><DocumentPanel processing={processing} progress={progress} ocrStatus={ocrStatus} parsedPages={parsedPages} /></div></div><RiskRow telemetry={telemetry} setSelectedEvent={setSelectedEvent} setView={setView} /><div className="lower-grid"><div className="panel activity-panel"><StreamPanel ocrStatus={ocrStatus} /></div><div className="panel prediction-panel"><PredictionPanel prediction={prediction} setPrediction={setPrediction} question={question} setQuestion={setQuestion} askPrediction={askPrediction} /></div></div></>
  }
