...
function Scanner({ onRead, onClose }: { onRead: (text: string) => Promise<void>; onClose: () => void }) {
...
function TransferQr({ frames }: { frames: string[] }) {
  ...
  useEffect(() => { if (paused || frames.length < 2) return; const timer = setInterval(() => setIndex(i => (i + 1) % frames.length), 900); return () => clearInterval(timer) }, [frames, paused])
  useEffect(() => { let live = true; QRCode.toDataURL(frames[index] || frames[0], { width: 440, margin: 4, errorCorrectionLevel: 'M' }).then(s => { if (live) setSrc(s) }); return () => { live = false } }, [frames, index])
...
export default function FieldCompanion({ receiver = false, wells = [], reports = [], onApplied }: { receiver?: boolean; wells?: string[]; reports?: WellReport[]; onApplied?: (notes: FieldNote[]) => void }) {
  ...
  return <section className={`field-companion ${receiver ? 'field-receiver' : 'field-phone'}`}>
    ...
    {message && <p className="field-message" role="status">{message}</p>}{scanning && <><Scanner onRead={readQr} onClose={() => setScanning(false)} /><p role="status">{progress}</p></>}{frames.length > 0 && <TransferQr frames={frames} />}
