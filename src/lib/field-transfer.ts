...
export async function framesFor(p: FieldPackage) {
  ...
  const count = Math.ceil(encoded.length / 650)
  if (count > 500) throw new Error('This batch is too large for QR. Export an update file instead.')
  return Array.from({ length: count }, (_, i) => JSON.stringify({ nwis: 1, id: p.id, hash, i, count, data: encoded.slice(i * 650, (i + 1) * 650) }))
}
export class FrameCollector {
  id = ''; hash = ''; count = 0; pieces = new Map<number, string>()
  async add(text: string): Promise<FieldPackage | null> {
    const f = JSON.parse(text)
    if (f.nwis !== 1 || typeof f.id !== 'string' || typeof f.hash !== 'string' || !/^[a-f0-9]{64}$/.test(f.hash) || !Number.isInteger(f.count) || f.count < 1 || f.count > 500 || !Number.isInteger(f.i) || f.i < 0 || f.i >= f.count || typeof f.data !== 'string' || f.data.length > 650) throw new Error('Not an NWIS transfer QR.')
    ...
    this.id = f.id; this.hash = f.hash; this.count = f.count; this.pieces.set(f.i, f.data)
